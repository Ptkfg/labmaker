...
from app import app
from models_equipamentos import EquipamentosConsumo

CSV_PATH = os.path.join("data", "levantamento_maker.csv")
ENCODING = "latin1"
SEP = ","
    ...
    with app.app_context():
        ...
        try:
            for i, row in df.iterrows():
                ...
                raw_validade = str(
                    row.get("validade") or
                    row.get("validade (no caso de não ter no Sias Files, considerar a data de fabricação)") or ""
                ).strip().lower()

                true_values = {"sim", "true", "1", "yes"}
                false_values = {"não", "false", "0", "no", "nao", "nvinfo", "n/a", "não informado", ""}

                if raw_validade in true_values:
                    validade = True
                elif raw_validade in false_values:
                    validade = False
                else:
                    print(f" Valor inválido para validade na linha {i}: '{raw_validade}' — será tratado como False")
                    validade = False  # não pula, mas força False
                ...
                item = EquipamentosConsumo(
                    ...
                    validade=True if validade is True else False,
            ...
            if not isinstance(validade, bool):
                validade = False
